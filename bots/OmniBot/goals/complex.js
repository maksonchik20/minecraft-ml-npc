const { GOAL_DESTROY } = require("../lib/goals");
...
module.exports = {
    ...
    priority: (bot, goal) => {
        ...
        goal.goals.forEach(_goal => {
            mx = Math.max(mx, _goal.priority.bind(_goal)(bot, _goal))
        });
        return mx;
    },
    execute: (bot, goal) => {
        ...
        goal.checkLoop = setInterval(() => {
            let to_start = null;
            let dstr = goal.goals.filter((_goal) => {return _goal.priority.bind(_goal)(bot, _goal) == GOAL_DESTROY})
            goal.goals = goal.goals.filter((_goal) => {return _goal.priority.bind(_goal)(bot, _goal) != GOAL_DESTROY})
            dstr.forEach((_goal) => {
